...
impl FileWalker {
  ...
  pub fn new(paths: impl Iterator<Item = PathBuf>) -> Self {
    ...
    for entry in paths {
      ...
      let walkdir = WalkDir::new(entry).into_iter().filter_entry(|entry| {
        !IGNORED.contains(&&*entry.file_name().to_string_lossy())
      });
